...
// @version      19.1.0
// @description  Блокировка рекламы Twitch через очистку M3U8 и модификацию GQL (кроме PlaybackAccessToken). Включает авто-перезагрузку при ошибках плеера.
// @author       ShmidtS
...
(function() {
    ...
    const RELOAD_STALL_THRESHOLD_MS = GM_getValue('TTV_AdBlock_StallThresholdMs', 8000);
    const RELOAD_BUFFERING_THRESHOLD_MS = GM_getValue('TTV_AdBlock_BufferingThresholdMs', 25000);
    ...
    if (BLOCK_NATIVE_ADS_SCRIPT) {
        ...
        AD_URL_KEYWORDS_BLOCK.push('player-ad-aws.js');
    }
    ...
    const AD_DOM_ELEMENT_SELECTORS_TO_REMOVE = [
        ...
        'div[class*="InjectLayout-sc-"] > div[class*="Layout-sc-"] > div[class*="InjectLayout-sc-"]',
    ...
    if (HIDE_COOKIE_BANNER) {
        AD_DOM_ELEMENT_SELECTORS_TO_REMOVE.push(COOKIE_BANNER_SELECTOR);
    }
    ...
    function logModuleTrace(moduleFlag, source, message, ...args) {
        if (moduleFlag) {
            const truncatedArgs = args.map(arg => typeof arg === 'string' && arg.length > 150 ? arg.substring(0, 150) + '...' : arg);
    ...
    if (cssToInject) {
        try { GM_addStyle(cssToInject); }
        catch (e) { logError('CSSInject', 'Не удалось применить CSS:', e); }
    }
    ...
    function parseAndCleanM3U8(m3u8Text, url = "N/A") {
        ...
        if (AD_SIGNIFIERS_GENERAL.some(sig => upperM3U8Text.includes(sig.toUpperCase()))) potentialAdContent = true;
        else if (AD_DATERANGE_REGEX && AD_DATERANGE_REGEX.test(m3u8Text)) potentialAdContent = true;
        else if (!AD_DATERANGE_REGEX && upperM3U8Text.includes("DATERANGE") && AD_DATERANGE_ATTRIBUTE_KEYWORDS.some(kw => upperM3U8Text.includes(kw))) potentialAdContent = true;

        if (!potentialAdContent) { return { cleanedText: m3u8Text, adsFound: false, linesRemoved: 0 }; }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (upperTrimmedLine.startsWith('#EXT-X-DATERANGE:')) {
                ...
                if (upperTrimmedLine.includes(AD_SIGNIFIER_DATERANGE_ATTR.toUpperCase()) || (AD_DATERANGE_REGEX && AD_DATERANGE_REGEX.test(trimmedLine))) {
                    isAdDateRange = true; removalReason = 'DATERANGE (attr/Regex)';
                } else if (AD_DATERANGE_ATTRIBUTE_KEYWORDS.some(kw => upperTrimmedLine.includes(kw))) {
                    isAdDateRange = true; removalReason = 'DATERANGE (Keyword)';
                }
                if (isAdDateRange) {
                    isAdSection = true; isAdMarkerLine = true; adsFound = true; shouldRemoveLine = true; currentAdMarkers.push(removalReason);
                }
            } else if (upperTrimmedLine.startsWith('#EXT-X-ASSET:')) {
                isAdSection = true; isAdMarkerLine = true; adsFound = true; shouldRemoveLine = true; removalReason = 'ASSET'; currentAdMarkers.push(removalReason);
            } else if (AD_SIGNIFIERS_GENERAL.some(sig => upperTrimmedLine.startsWith(sig.toUpperCase()) && (sig.toUpperCase().includes('CUE-OUT') || sig.toUpperCase().includes('AD-SIGNAL')))) {
                removalReason = 'Start (' + upperTrimmedLine.split(':')[0] + ')';
                isAdSection = true; isAdMarkerLine = true; adsFound = true; shouldRemoveLine = true; currentAdMarkers.push(removalReason);
            } else if (AD_SIGNIFIERS_GENERAL.some(sig => upperTrimmedLine.startsWith(sig.toUpperCase()) && (sig.toUpperCase().includes('CUE-IN') || sig.toUpperCase().includes('AD-SIGNAL')))) {
                if (isAdSection) {
                    isAdSection = false; isAdMarkerLine = true; shouldRemoveLine = true; removalReason = 'End (' + upperTrimmedLine.split(':')[0] + ')'; if (currentAdMarkers.length > 0) currentAdMarkers.pop();
                } else {
                    shouldRemoveLine = true; isAdMarkerLine = true; removalReason = 'Orphan ' + upperTrimmedLine.split(':')[0]; logWarn(context, `(${urlShort}): Найден ${removalReason} [${i + 1}].`);
                }
            } else if (isAdSection && !trimmedLine.startsWith('#') && trimmedLine !== '') {
                shouldRemoveLine = true; removalReason = 'Segment';
            } else if (isAdSection && upperTrimmedLine.startsWith('#EXT-X-DISCONTINUITY')) {
                shouldRemoveLine = true; isAdMarkerLine = true; removalReason = 'Discontinuity';
            }

            if (shouldRemoveLine) { linesRemoved++; }
            else { cleanLines.push(line); }
        }

        if (isAdSection && currentAdMarkers.length > 0) logWarn(context, `(${urlShort}): M3U8 завершился в секции рекламы (Маркеры: ${currentAdMarkers.join(', ')}).`);
        const cleanedText = cleanLines.join('\n');
        if (adsFound) { logCoreDebug(context, `(${urlShort}): Завершено. Удалено ${linesRemoved} строк.`); }
        ...
        const workerCodeString = (function() {
            ...
            function workerLogM3U8Trace(message, ...args) {
                if (LOG_M3U8_CLEANING_DEBUG_WORKER) {
                    ...
                    console.debug(LOG_PREFIX_WORKER_BASE + ' [M3U8 Relay] TRACE:', message, ...truncatedArgs);
                }
            }
            ...
            function installWorkerFetchHook() {
                        ...
                        return new Promise((resolveWorkerPromise, rejectWorkerPromise) => {
                            ...
                            const onMessageFromMain = (event) => {
                                if (event.data && event.data.type === 'ADBLOCK_M3U8_RESPONSE' && event.data.id === messageId) {
                                    ...
                                    if (event.data.error) {
                                        workerLogError('Error from main: ' + urlShort + ':', event.data.error);
                                        rejectWorkerPromise(new TypeError(event.data.error));
                                    } else {
                                        workerLogM3U8Trace('Received processed M3U8: ' + urlShort);
                            ...
                            try {
                                ...
                                workerLogM3U8Trace('Posted request to main: ' + urlShort);
                            } catch (e) {
                                self.removeEventListener('message', onMessageFromMain);
                                workerLogError('Failed postMessage to main:', e);
             ...
             self.addEventListener('message', function(e) {
                ...
                else if (e.data?.type === 'EVAL_ADBLOCK_CODE') { workerLogCoreDebug("[WorkerInject:Msg] Received EVAL_ADBLOCK_CODE."); }
                else if (e.data && e.data.type !== 'ADBLOCK_M3U8_RESPONSE') { workerLogCoreDebug("[WorkerInject:Msg] Received unhandled message:", e.data?.type || e.data); }
            });

            if (installWorkerFetchHook()) {
                try { self.postMessage({ type: 'ADBLOCK_WORKER_HOOKS_READY', method: 'eval_fetch_hook_m3u8_relay' }); }
                catch(e) { workerLogError("Error sending ADBLOCK_WORKER_HOOKS_READY:", e); }
            } else {
                 workerLogError("Failed to install fetch hook!");
                 try { self.postMessage({ type: 'ADBLOCK_WORKER_HOOKS_READY', method: 'eval_fetch_hook_m3u8_relay_failed' }); } catch(e){}
            }
    ...
    function modifyGqlResponse(responseText, url) {
        ...
        try {
            ...
            for (const opData of opDataArray) {
                ...
                if (operationName === GQL_OPERATION_ACCESS_TOKEN || operationName === GQL_OPERATION_ACCESS_TOKEN_TEMPLATE) {
                    logModuleTrace(LOG_GQL_MODIFY_DEBUG, context, `Пропуск модификации ${operationName}.`);
                }
                    ...
                    if (opData.data?.viewerAdFreeConfig?.isAdFree === false) {
                     ...
                     logModuleTrace(LOG_GQL_MODIFY_DEBUG, context, `Обработка ${operationName}...`);
                     if (opData.data && (opData.data.ads || opData.data.clientAd || opData.data.companionSlots || opData.data.playerAdParams)) {
                        opData.data = {};
                        currentOpModified = true;
                        logModuleTrace(LOG_GQL_MODIFY_DEBUG, context, `${operationName}: data -> {}`);
                    }
                }
                    ...
                    logModuleTrace(LOG_GQL_MODIFY_DEBUG, context, `Пропуск GQL: ${operationName}`);
                }
    ...
    async function fetchOverride(input, init) {
        ...
        try {
            ...
            if (blockedUrlKeywordsSet.has(requestHostname) || AD_URL_KEYWORDS_BLOCK.some(k => lowerCaseUrl.includes(k))) {
                if (lowerCaseUrl.includes('nat.min.js') && !BLOCK_NATIVE_ADS_SCRIPT) {/* Skip */}
                else { logWarn(context, `БЛОКИРОВКА Fetch (${method}): ${urlShort}`); isBlockedByKeyword = true; }
            }
        } catch(urlError) {
             if (!(urlError.message.includes("Invalid URL") && requestUrl.startsWith("blob:"))) { logWarn(context, `URL Error: ${requestUrl}: ${urlError.message}`); }
            if (AD_URL_KEYWORDS_BLOCK.some(k => lowerCaseUrl.includes(k))) {
                 if (lowerCaseUrl.includes('nat.min.js') && !BLOCK_NATIVE_ADS_SCRIPT) {/* Skip */}
                 else { logWarn(context, `БЛОКИРОВКА Fetch (${method}) (fallback): ${urlShort}`); isBlockedByKeyword = true; }
            }
        }
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function(response) {
                        if (response.status >= 200 && response.status < 300) {
                            ...
                            if (adsFound) { logCoreDebug(context, `GM_xmlhttpRequest: ${urlShort} ОЧИЩЕН. ${linesRemoved} строк удалено.`); }
                            ...
                            logError(context, `GM_xmlhttpRequest ${urlShort} не удался: ${response.status} ${response.statusText}`);
        ...
        try {
            ...
            logModuleTrace(CORE_DEBUG_LOGGING, context, `Original fetch done for ${urlShort}, Status: ${originalResponse.status}`);

            if (isGqlRequest && method === 'POST' && originalResponse.ok && MODIFY_GQL_RESPONSE) {
                const contextGQL = 'GQL:FetchResp'; let responseText;
                try { responseText = await originalResponse.clone().text(); if (!responseText) return originalResponse; }
                catch (e) {
                    if (e.name === 'AbortError') { logModuleTrace(LOG_GQL_MODIFY_DEBUG, contextGQL, `Clone aborted: ${urlShort}`); return originalResponse; }
                    else { logError(contextGQL, `Clone/text error: ${urlShort}:`, e); return originalResponse; }
                }
                const modifiedText = modifyGqlResponse(responseText, requestUrl);
                if (modifiedText !== responseText) {
                    const newHeaders = new Headers(originalResponse.headers); newHeaders.delete('Content-Length');
            ...
            if (error instanceof TypeError && (error.message.includes('fetch') || error.message.includes('Network') || error.message.includes('aborted'))) {
                logWarn(context, `Fetch failed for ${urlShort}: ${error.message}`);
                 return Promise.resolve(new Response(null, { status: 0, statusText: `Simulated Network Error` }));
            } else if (error instanceof DOMException && error.name === 'AbortError') {
                logModuleTrace(CORE_DEBUG_LOGGING, context, `Fetch aborted: ${urlShort}`);
                 return Promise.resolve(new Response(null, { status: 0, statusText: "Fetch aborted" }));
            }
            else { logError(context, `Unexpected fetch error for ${urlShort}:`, error); }
            throw error;
        }
    }
    ...
    function handleMessageFromWorkerGlobal(event) {
        ...
        if (msgData && typeof msgData.type === 'string') {
            ...
            if (msgData.type === 'ADBLOCK_WORKER_HOOKS_READY') {
                logCoreDebug('WorkerInject:Confirm', `HOOKS READY: ${workerLabel} (Method: ${msgData.method || 'N/A'})`);
                const workerState = hookedWorkers.get(workerLabel);
                if (workerState) workerState.hooksReady = true; else logWarn('WorkerInject:Confirm', `HOOKS_READY from unknown Worker: ${workerLabel}`);
            } else if (msgData.type === 'PONG') {
                const workerState = hookedWorkers.get(workerLabel);
                if (workerState) workerState.lastPong = Date.now();
                logModuleTrace(LOG_WORKER_INJECTION_DEBUG, 'WorkerInject:Pinger', `PONG from ${workerLabel}`);
            } else if (msgData.type === 'ADBLOCK_FETCH_M3U8_REQUEST') {
                logCoreDebug('MainWorkerComm', `Received M3U8 request from ${workerLabel}: ${msgData.url}`);
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function(response) {
                        ...
                        if (response.status >= 200 && response.status < 300) {
                            ...
                            logModuleTrace(LOG_M3U8_CLEANING_DEBUG, 'MainWorkerComm', `Sending success response for ${id} (AdsFound: ${adsFound})`);
                        } else {
                            postData.error = `Failed GM_xmlhttpRequest: ${response.status} ${response.statusText}`;
                            postData.status = response.status; postData.statusText = response.statusText;
                            logError('MainWorkerComm', `GM_xmlhttpRequest error for ${id}: ${postData.error}`);
                        }
                         try { workerInstance.postMessage(postData); } catch (e) { logError('MainWorkerComm', `postMessage error to ${workerLabel}: ${e}`);}
                    },
                    onerror: function(error) { logError('MainWorkerComm', `Network error for ${id}:`, error); try { workerInstance.postMessage({ type: 'ADBLOCK_M3U8_RESPONSE', id: id, error: 'Network error', status: 0, statusText: 'Network Error' }); } catch (e) { logError('MainWorkerComm', `postMessage error to ${workerLabel}: ${e}`);} },
                    ontimeout: function() { logError('MainWorkerComm', `Timeout for ${id}.`); try { workerInstance.postMessage({ type: 'ADBLOCK_M3U8_RESPONSE', id: id, error: 'Timeout', status: 0, statusText: 'Timeout' }); } catch (e) { logError('MainWorkerComm', `postMessage error to ${workerLabel}: ${e}`);} }
                });
            }
        }
    }
    function addWorkerMessageListeners(workerInstance, label) {
        ...
        workerInstance.addEventListener('error', (e) => { logError('WorkerInject:Error', `Error inside Worker (${label}):`, e.message, `(${e.filename}:${e.lineno}:${e.colno})`); hookedWorkers.delete(label); }, { passive: true });
        logCoreDebug('WorkerInject:MsgListenerSetup', `Listeners added for Worker (${label})`);
    }
    function hookWorkerConstructor() {
        const context = 'WorkerInject:HookSetup';
        if (!INJECT_INTO_WORKERS) { logWarn(context, "Worker Inject disabled."); return; }
        if (unsafeWindow.Worker?.hooked_by_adblock_ult) { logCoreDebug(context, "Worker already hooked."); return; }
        if (typeof OriginalWorker === 'undefined') { logError(context, 'Original Worker not found.'); return; }
        ...
        unsafeWindow.Worker = class HookedWorker extends OriginalWorker {
            constructor(scriptURL, options) {
                ...
                logModuleTrace(LOG_WORKER_INJECTION_DEBUG, context, `New Worker: ${workerLabel} (Type: ${isBlobURL ? 'Blob' : 'Script'}${isIVSWorker ? ', IVS' : ''})`);
                    ...
                    if (!isIVSWorker) {
                        setTimeout(() => {
                            ...
                            if (workerState && !workerState.hooksReady) {
                                logWarn(context, `Worker (${workerLabel}) didn't signal HOOKS_READY. Attempting postMessage injection...`);
                                try {
                                    const codeToInject = getWorkerInjectionCode();
                                    logCoreDebug(context, `Sending EVAL_ADBLOCK_CODE to Worker (${workerLabel})...`);
                                    newWorkerInstance.postMessage({ type: 'EVAL_ADBLOCK_CODE', code: codeToInject });
                                } catch(e) { logError(context, `postMessage fallback error for ${workerLabel}:`, e); hookedWorkers.delete(workerLabel); }
                            }
                        }, 250);
                    }
                } catch (constructorError) { logError(context, `HookedWorker constructor error for ${workerLabel}:`, constructorError); hookedWorkers.delete(workerLabel); throw constructorError; }
                return newWorkerInstance;
            }
        };
        try { Object.keys(OriginalWorker).forEach(key => { if (!unsafeWindow.Worker.hasOwnProperty(key)) { try { unsafeWindow.Worker[key] = OriginalWorker[key]; } catch(e){} } }); Object.setPrototypeOf(unsafeWindow.Worker, OriginalWorker); Object.setPrototypeOf(unsafeWindow.Worker.prototype, OriginalWorker.prototype); } catch (e) { logError(context, "Failed to restore Worker prototype:", e); }
    ...
    function removeDOMAdElements(contextNode = document) {
        ...
        AD_DOM_ELEMENT_SELECTORS_TO_REMOVE.forEach(selector => {
            ...
            try {
                ...
                elements.forEach(el => {
                    if (el && el.parentNode) {
                        if (el.querySelector('video') || el.closest(PLAYER_SELECTORS_IMPROVED.join(','))) { return; }
                        logModuleTrace(ATTEMPT_DOM_AD_REMOVAL, 'DOMRemove', `Removing element by selector: "${selector}"`);
                        el.parentNode.removeChild(el); removedCount++;
                    }
                });
            } catch (e) { logWarn('DOMRemove', `Error removing "${selector}":`, e); }
        });
    ...
    function startAdRemovalObserver() {
        ...
        logCoreDebug(context, `Observing DOM changes in:`, observedNode);
                ...
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    for (const node of mutation.addedNodes) {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                             for (const adSelector of AD_DOM_ELEMENT_SELECTORS_TO_REMOVE) { if (adSelector === COOKIE_BANNER_SELECTOR && !HIDE_COOKIE_BANNER) continue; if (node.matches && node.matches(adSelector)) { potentialAdNodeAdded = true; break; } }
                             if (!potentialAdNodeAdded) { for (const adSelector of AD_DOM_ELEMENT_SELECTORS_TO_REMOVE) { if (adSelector === COOKIE_BANNER_SELECTOR && !HIDE_COOKIE_BANNER) continue; if (node.querySelector(adSelector)) { potentialAdNodeAdded = true; break; } } }
                            if (potentialAdNodeAdded) break;
                            if (node.tagName === 'IFRAME' && AD_URL_KEYWORDS_BLOCK.some(keyword => node.src?.toLowerCase().includes(keyword))) { potentialAdNodeAdded = true; break; }
                        }
                        if (potentialAdNodeAdded) break;
                    }
                }
        ...
        try { adRemovalObserver.observe(observedNode, observerConfig); removeDOMAdElements(observedNode); logCoreDebug(context, 'DOM Observer started.'); }
    ...
    setTimeout(() => {
        if (!videoElement && (document.readyState === 'complete' || document.readyState === 'interactive')) {
            logWarn('Init', 'Fallback player check after 5s...');
            setupPlayerMonitor();
        }
        if (ATTEMPT_DOM_AD_REMOVAL && !adRemovalObserver) {
             logWarn('Init', 'Fallback AdRemovalObserver start after 5s...');
